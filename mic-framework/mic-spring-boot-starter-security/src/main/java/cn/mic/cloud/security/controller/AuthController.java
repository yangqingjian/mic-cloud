...
public class AuthController {
	...
	public Result<TokenResult> login(@Valid @RequestBody LoginAuthRequest loginAuthRequest) {
		Optional<LoginTypeInterface> loginInterfaceOptional = loginInterfaces.stream()
				.filter(temp -> temp.support(loginAuthRequest)).findFirst();
		 ...
		 * 同是也存入redis(把前缀去掉)
		 */
		LoginTokenRedisStoreResponse response = loginAuthInterface.redisStoreToken(LoginTokenRedisStoreRequest.builder()
				.key(token).expireSeconds(securityCommonConfig.getExpireTimeSeconds()).loginUser(loginUser).build());
